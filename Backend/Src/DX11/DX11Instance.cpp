...
{
  CGE_TRY(CreateDeviceAndSwapchain(createInfo));
  return TResult::Okay();
}
...
TResult CInstance::CreateTexture(const TTextureCreateInfo& createInfo, std::unique_ptr<ITexture>& pTexture)
{
  pTexture = std::make_unique<CTexture>(*this);
...
TResult CInstance::CreateDeviceAndSwapchain(const TCreateInfo& createInfo)
{
